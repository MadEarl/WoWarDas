...
class GpxFileWriter(private val wayPointList: MutableList<Entry>) {
    ...
    init {
        ...
        metaDataBuilder.append("        <name>WoWarDas</name>\n")
        ...
        metaDataBuilder.append("            <link href=\"https://github.com/MadEarl/WoWarDas/\"><text>WoWarDas @ GitHub</text></link>\n")
        metaDataBuilder.append("        </author>\n")
        metaDataBuilder.append("    </metadata>\n")
    ...
    private fun buildGpxFile(): String {
        ...
        for (entry in wayPointList) {
            val commentString = if (entry.comment != "") entry.comment.toString() else entry.id.toString()
            ...
            gpxBuilder.append("            <cmt>${commentString}</cmt>\n")
